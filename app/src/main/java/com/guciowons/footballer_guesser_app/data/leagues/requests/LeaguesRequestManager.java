...
public class LeaguesRequestManager {
    ...
    private void getLeagues(LeagueRepository leagueRepository, JSONArray leaguesJson){
        ...
        for(int i = 0; i< leaguesJson.length(); i++) {
            try {
                convertLeague(leagueRepository, leaguesJson, i);
    ...
    private void convertLeague(LeagueRepository leagueRepository, JSONArray leaguesJson, Integer i) throws JSONException {
        ...
        leagueRepository.getRequestQueue().add(new ImageRequest(league.getUrl(),
                leagueLogo -> setLeagueLogo(leagueRepository, leaguesJson, league, leagueLogo),
                150, 150, ImageView.ScaleType.CENTER, null,
                error -> leagueRepository.setError("One or more leagues have not been fetched!")));
    }

    private void setLeagueLogo(LeagueRepository leagueRepository, JSONArray leaguesJson, League league, Bitmap leagueLogo){
